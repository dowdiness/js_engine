...
const {
  ...
  payloadFingerprint,
  validateProbeSpec,
} = require("./jetstream3_v8_probe.js");
...
test("a V8 probe fixes one reproducible candidate", () => {
  ...
  assert.throws(
    () =>
      validateProbeSpec({
        ...spec,
        engine: { ...spec.engine, version: "15.4.77@canary" },
      }),
    /engine.version/,
  );
  assert.throws(
    () =>
      validateProbeSpec({
        ...spec,
        engine: { ...spec.engine, jsvu_engine: "javascriptcore" },
      }),
    /jsvu_engine must be v8/,
  );
});

test("the source-controlled V8 generation is fixed", () => {
  const spec = validateProbeSpec(
    JSON.parse(
      fs.readFileSync(path.join(__dirname, "jetstream3_v8_probe.json"), "utf8"),
    ),
  );

  assert.equal(spec.generation, 1);
  assert.equal(spec.engine.version, "15.4.77");
  assert.equal(spec.workload, "navier-stokes");
});

test("V8 uses its standard shell without a repository adapter", () => {
  const tempRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "jetstream3-v8-compatible-"),
  );
  try {
    const engineRoot = path.join(tempRoot, "v8-15.4.77");
    fs.mkdirSync(engineRoot);
    fs.writeFileSync(path.join(engineRoot, "v8-15.4.77"), "installed payload\n");
    ...
    assert.deepEqual(commands[0], [
      path.join(engineRoot, "v8-15.4.77"),
