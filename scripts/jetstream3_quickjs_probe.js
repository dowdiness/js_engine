...
function usage() {
  return `Usage: node scripts/jetstream3_quickjs_probe.js [options]
...
function validateProbeSpec(spec) {
  ...
  for (const field of [
    ...
    requireString(engine[field], `engine.${field}`);
  }
  ...
  if (
    ...
    !/^[0-9]+(?:[.-][0-9A-Za-z]+)*$/.test(engine.version)
  ) {
    throw new Error("engine.version must be exact");
  }
  if (
    path.isAbsolute(engine.executable) ||
    engine.executable.split(/[\\/]/).includes("..")
  ) {
  ...
  if (!/^[0-9a-f]{64}$/.test(engine.payload_sha256 || "")) {
...
function parseArgs(argv) {
  const options = {
    output: "jetstream3-quickjs-probe.json",
...
function main(argv, dependencies = {}) {
  ...
  const actualFingerprint = payloadFingerprint(options.engineRoot);
  if (actualFingerprint !== engine.payload_sha256) {
    throw new Error(
      `payload fingerprint mismatch: expected ${engine.payload_sha256}, got ${actualFingerprint}`,
    );
  }
  const executable = path.join(options.engineRoot, engine.executable);
  ...
  const report = {
    ...
    engine: {
      id: engine.id,
      jsvu_engine: engine.jsvu_engine,
      version: engine.version,
      payload_sha256: actualFingerprint,
      shell_adapter: engine.shell_adapter,
      compatibility,
    },
    environment: {
