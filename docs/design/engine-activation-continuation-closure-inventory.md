...
2026-08-05 after the first bound-argument ordinary direct-return slice in #813
under #800.
...
| Direct-return leaf call | At the base activation, the saved return consumer enters one separately sealed ordinary function with either zero arguments or one closed literal argument | A closed literal or the leaf's own prepared parameter binding resumes the entry function's return consumer once; throw and runtime abrupt completions bypass that consumer | Simple ordinary-function parameter preparation | Both named callees are canonical global declarations with sealed identity, closure, body, params, source, and realm provenance; the leaf body is exactly one admitted return or throw and a bound value must match the sealed call argument |
...
| Call, return, and binary work | The exact numeric recipe retains the left `Number`, performs `+`, `-`, or `===` in the closed numeric set, delivers the callee result once, and routes the admitted return. The exact ordinary direct-return recipes route recursive and sealed leaf calls, including one closed argument and its prepared formal binding, through explicit return continuations | Arbitrary calls, arguments, operators, coercions, return expressions, and throw expressions outside those exact recipes are residuals |
...
|---|---|---|
| Public production | The exact #616 depth-256 programs (including the #790 retained-argument comma workload), exact direct numeric `call_value`, exact #809/#811/#813 ordinary direct-return recipes, admitted protected numeric completions, and the two exact protected-control roots pass through public adapters | General programs or callable families are stack-safe |
