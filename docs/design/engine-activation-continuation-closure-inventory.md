...
2026-08-07 after #608 admitted two-argument mixed ordered-source plans with one
closed argument and one recursive result, #823 admitted one exact ordered
two-helper result pipeline, and #828 generalized finite non-empty linear
result-fed helper chains. The required public workload fixes
`[ClosedArgument("ignored"), RecursiveResult(f(n - 1))]`; #817 remains the
separate single-recursive-result recipe, following the closed ordered-argument
...
| Direct-return self recursion | The root and recursive `DispatchCallRequest` values enter the sealed ordinary `Object(UserFunc)` for `f`; #819 additionally admits a sealed zero-parameter `entry()` wrapper whose body calls `f(256)` | The recursive result crosses function-exit cleanup and resumes the saved return consumer exactly once; the wrapper adds one ordinary activation and preserves the existing `DispatchCompleteReturn` continuation | None | The non-strict named callees are hoisted declarations; `this` is `Undefined`; direct entry arguments are the finite integer `Number(256)` while the wrapper entry has no arguments; the base return or throw is a closed literal; and the recursive expression is exactly `f(n - 1)` |
| Direct-return leaf call | At the base activation, or while a recursive result unwinds, the saved return consumer enters one separately sealed ordinary function with zero, one, or two ordered arguments | A closed literal or the leaf's selected prepared parameter binding resumes the entry function's return consumer once; a recursive result fills its owned source slot before the call; throw and runtime abrupt completions bypass that consumer | Simple ordinary-function parameter preparation | Both named callees are canonical global declarations with sealed identity, closure, body, ordered params, source, and realm provenance; each closed argument owns its literal and location, a result-fed argument owns the inner recursive-call location, the leaf body is exactly one admitted return or throw, and every prepared binding matches the actual argument snapshot by JavaScript value/identity semantics. #608 additionally admits two-slot mixed vectors with exactly one closed source and one recursive-result source in either ordered position; the required public workload fixes `leaf("ignored", f(n - 1))`. #828 additionally admits a finite non-empty linear chain of these helper calls, collected iteratively from outer to inner and sealed in inner-to-outer declaration order, with one carried-result slot and closed literals at every stage; the required public workload fixes `outer("ignored", middle(inner(f(n - 1))))`. Dynamic, property, spread, transformed, reordered, duplicate, and recursively-defined helper shapes remain outside the slice. |
...
| Call, return, and binary work | The exact numeric recipe retains the left `Number`, performs `+`, `-`, or `===` in the closed numeric set, delivers the callee result once, and routes the admitted return. The exact ordinary direct-return recipes route recursive and sealed leaf calls, including an owned ordered vector of up to two closed arguments, a single recursive-result source, #608's two-slot mixed ordered-source plans, and #828's finite non-empty linear carried-result chain, through explicit return continuations and prepared formal bindings | Arbitrary calls, dynamic argument lists, multiple carried results, operators, coercions, return expressions, and throw expressions outside those exact recipes are residuals |
...
- the defensive snapshot of the actual argument array used to validate a
  prepared direct-return leaf binding;
- the sealed direct-return root role, including the optional wrapper's
  declaration index, exact body and call locations, global binding/mirror,
  UserFunc identity, closure, and realm/source provenance;
- the ordered direct-return helper catalog and defensive expression-plan
  templates. For #828 the catalog contains every finite non-empty helper chain
  in declaration order, and each template owns exactly one carried-result
  index plus its closed argument values and call location;
...
|---|---|---|
| Public production | The exact #616 depth-256 programs (including the #790 retained-argument comma workload), exact direct numeric `call_value`, exact #809/#811/#813/#815/#817/#819 ordinary direct-return recipes, #608's two-slot mixed ordered-source family with required `leaf("ignored", f(n - 1))` public evidence, #828's finite non-empty linear helper-chain family with required three-helper source `return outer("ignored", middle(inner(f(n - 1))));`, admitted protected numeric completions, and the two exact protected-control roots pass through public adapters | General programs or callable families are stack-safe |
| Direct shell and lifecycle | Exact admission/sealing, activation identity, normal/return/guest-throw/runtime-abrupt cleanup, property-scope LIFO restoration, observation acceptance/rejection/release, and finalizer-before-release ordering; direct-shell/lifecycle observations show that #817's single-result recipe and #608's mixed-vector recipe each have 513 ordinary activations (257 `f` plus 256 leaf calls), #823's two-helper recipe has 769 (257 `f`, 256 `inner`, and 256 `outer`), #828's three-helper workload has 1,025 (257 `f`, 256 each `inner`, `middle`, and `outer`), all with maximum active guest depth 257, while #819 has 514 (1 `entry`, 257 `f`, and 256 `id`) | A shell state without public admission is production reachable |
