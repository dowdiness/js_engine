...
| `ArrayPushSpread`, `ArgPushSpread` | `runtime.begin_executor_iterable_spread` and `runtime.executor_activation_iterable_spread_request` in their dispatch arms; destinations are `AppendArraySpread` / `AppendArgumentSpread`. | Runtime owns iterable lookup, iterator calls, result validation, and abrupt completion in a private activation frame; the VM appends one closed batch to its concrete sink. | `ManagedSuspension` |
...
| `Call`, `CallWithReceiver`, `CallSpread`, `CallSpreadWithReceiver` | `executor_activation_call` in their dispatch arms; destination is `PushValue`. | Every form returns an existing typed call request; spread iteration is separately managed at `ArgPushSpread`, before this instruction. | `ManagedSuspension` |
| `Construct` | `executor_activation_construct` in its dispatch arm; destination is `PushValue`. | Fixed-arity construction remains managed. | `ManagedSuspension` |
| `ConstructSpread` | `executor_activation_construct` remains the VM arm, but selection rejects the instruction before a `BytecodeFrame` is built. | The shared dynamic-argument lowering must not admit constructor spread when `ArgPushSpread` is managed for calls. | `UnsupportedBeforeActivation(ConstructSpread)` |
