...
**Overall** — CI run [24730849102](https://github.com/dowdiness/js_engine/actions/runs/24730849102) on tip `f89898a` (v0.2.0), 2026-04-21. Each test file is run twice (strict + non-strict); the two are reported separately because summing them would double-count files.

| Mode | Discovered | Skipped | Executed | Passed | Failed | Passed / Executed | Passed / Discovered |
|---|---|---|---|---|---|---|---|
| strict | 44,986 | 18,270 | 26,601 | 23,054 | 3,547 | **86.7%** | 51.2% |
| non-strict | 47,692 | 18,811 | 28,766 | 24,467 | 4,299 | **85.1%** | 51.3% |
...
- The *Passed / Executed* column (86.7% / 85.1%) is what's usually reported as "test262 pass rate", but it excludes those skipped tests. The *Passed / Discovered* column (~51%) is the honest spec-coverage figure — it drops as new ES features are added to the suite faster than we implement them.

---

## Per-Edition Pass Rates

Tests classified by the ECMAScript edition in which each feature reached Stage 4. Generated by `scripts/classify-by-edition.py` against CI run [24730849102](https://github.com/dowdiness/js_engine/actions/runs/24730849102) (2026-04-21, tip `f89898a`, v0.2.0). Feature → edition map lives at the top of that script.
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| Pre-ES2015 (baseline) | 13,291 | 2 | 13,256 | 11,867 | 1,389 | 33 | 89.5% | 89.3% |
| ES2015 | 10,327 | 160 | 10,096 | 8,646 | 1,450 | 71 | 85.6% | 83.7% |
...
| **Total** | **44,986** | **18,270** | **26,601** | **23,054** | **3,547** | **115** | **86.7%** | **51.2%** |

### non-strict

| Edition | Discovered | Skipped | Executed | Passed | Failed | Timeout/Err | Passed / Executed | Passed / Discovered |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| Pre-ES2015 (baseline) | 13,930 | 5 | 13,892 | 12,304 | 1,588 | 33 | 88.6% | 88.3% |
| ES2015 | 10,832 | 169 | 10,592 | 8,962 | 1,630 | 71 | 84.6% | 82.7% |
| ES2016 | 100 | 0 | 93 | 71 | 22 | 7 | 76.3% | 71.0% |
...
| **Total** | **47,692** | **18,811** | **28,766** | **24,467** | **4,299** | **115** | **85.1%** | **51.3%** |
...
From CI run 24730849102 (2026-04-21, v0.2.0): **18,270 skipped strict + 18,811 skipped non-strict** — ~40% of the discovered tests in each mode. These are excluded from the Passed / Executed column but explain the gap between ~86% and ~51% spec coverage. Per-file counts below; the underlying file list is the same for both modes.
