...
**Overall** — CI run [24885185424](https://github.com/dowdiness/js_engine/actions/runs/24885185424) on tip `b225cda`, 2026-04-24. Each test file is run twice (strict + non-strict); the two are reported separately because summing them would double-count files.

| Mode | Discovered | Skipped | Executed | Passed | Failed | Passed / Executed | Passed / Discovered |
|---|---|---|---|---|---|---|---|
| strict | 44,986 | 18,270 | 26,598 | 23,359 | 3,239 | **87.8%** | 51.9% |
| non-strict | 47,692 | 18,811 | 28,767 | 24,809 | 3,958 | **86.2%** | 52.0% |
...
- The *Passed / Executed* column (87.8% / 86.2%) is what's usually reported as "test262 pass rate", but it excludes those skipped tests. The *Passed / Discovered* column (~52%) is the honest spec-coverage figure — it drops as new ES features are added to the suite faster than we implement them.
...
- **Pre-ES2015 (baseline)** is at ~91% *Passed / Executed* (strict ~1,193 fails, non-strict ~1,394 fails — see CI for current numbers). Largest absolute pool of "basic JS" failures and the biggest ROI target. Dominated by `built-ins/Array` (~390/399) and `built-ins/Object` (~240/246) in both modes. Failures cluster into roughly: Array internals (post-Stage-C fast-path audit pending), Object descriptor metadata round-trip (~95 each mode, "should be configurable / not be writable"), residual missing strict-mode TypeErrors, an ~23-test "over-throwing TypeError" cohort that likely shares one helper, ~35 Unicode `IdentifierStart/Part` lexer gaps, ~38 algorithmic timeouts, plus 113 RegExp pre-existing engine limits and 31 sloppy-mode `arguments`-aliasing tests. Without RegExp / sloppy-arguments / timeouts, the realistic ceiling is ~96% / ~95% *Passed / Executed*. See `docs/agent-todo.md` § "Pre-ES2015 baseline push (2026-05-07 classification)" for the prioritized cluster plan.
